...
def processVideo (lectureName, fileName):
    ...
    detectionCode = subprocess.call('python ' + detectionArgs, stdout=FNULL, shell=True)
    print ('Detection ended with exit code ' + str(detectionCode))

    sortingCode = subprocess.call('python ' + sortingArgs, stdout=FNULL, shell=True)
    print ('Sorting ended with exit code ' + str(sortingCode))

    extractionCode = subprocess.call('python ' + extractionArgs, stdout=FNULL, shell=True)
